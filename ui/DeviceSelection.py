...
class DeviceSelectionUI(tk.CTkToplevel):
    ...
    def __init__(self, root):
        ...
        self.__create_startup_widgets()
        self.resizable(width=False, height=False)
        self.protocol("WM_DELETE_WINDOW", self.master.destroy)
        UIHelper.set_window_icon(self)

    def __create_startup_widgets(self):
        ...
        self.startup_label.grid(pady=10, sticky="nsew")
        self.adb_connection_menu = tk.CTkOptionMenu(self, command=self.__show_current_screenshot)
        self.adb_connection_menu.grid(pady=10, sticky="nsew")
        self.device_refresh_button = tk.CTkButton(self, text="Refresh Device List", command=self.__refresh_device_ui)
        self.device_refresh_button.grid(pady=10, sticky="nsew")
        self.startup_button = tk.CTkButton(self, text="Start Application", command=self.__launch_main_window)
        self.startup_button.grid(pady=10, sticky="nsew")
        # Get active devices and refresh UI element
        self.__refresh_device_ui()

    def __refresh_device_ui(self):
        self.__refresh_adb_device_list()
        if serial_and_image_dict:
            ...
            self.__show_current_screenshot(serial_name_list[0])
        else:
    ...
    def __refresh_adb_device_list(self):
